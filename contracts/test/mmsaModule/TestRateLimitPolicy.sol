...
contract TestRateLimitPolicy is PolicyBase {
    ...
    mapping(bytes32 id => mapping(address kernel => RateLimitConfig))
        public rateLimitConfigs;
    ...
    function checkUserOpPolicy(
        ...
        rateLimitConfigs[id][msg.sender].startAt = ValidAfter.wrap(
            ValidAfter.unwrap(config.startAt) + config.interval
        );
    ...
    function _policyOninstall(
        ...
        rateLimitConfigs[id][msg.sender] = RateLimitConfig(
            delay,
            count,
            ValidAfter.wrap(startAt)
        );
