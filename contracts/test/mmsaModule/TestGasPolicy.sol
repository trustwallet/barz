...
contract TestGasPolicy is PolicyBase {
    ...
    mapping(bytes32 id => mapping(address => GasPolicyConfig))
    ...
    function _policyOninstall(
        ...
        (uint256 allowed, bool enforcePaymaster, address allowedPaymaster) = abi
            .decode(_data, (uint256, bool, address));

        gasPolicyConfig[id][msg.sender] = GasPolicyConfig(
            allowed,
            enforcePaymaster,
            allowedPaymaster
        );
