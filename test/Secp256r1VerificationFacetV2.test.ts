...
import { AccountFacet, DiamondCutFacet, Barz, Secp256r1VerificationFacetV2, SecurityManager, FacetRegistry, DiamondLoupeFacet, TokenReceiverFacet, DefaultFallbackHandler } from '../typechain-types'
import { adjustS, fillUserOpDefaults, getUserOpHash, signUserOpR1Curve } from './utils/UserOp'
import { getChainId, generateKeyPair, guardianSecurityPeriod, guardianSecurityWindow, recoveryPeriod, lockPeriod, approvalValidationPeriod, migrationPeriod } from './utils/helpers'

import { UserOperation } from './utils/UserOperation'
import { keccak256 as keccak256_buffer } from 'ethereumjs-util'
...
import { createAccountOwner, fund, verificationGasLimit, maxFeePerGas, getMessageHash } from './utils/testutils'
...
describe('Secp256r1 Verification Facet', () => {
    ...
    let r1Barz: Secp256r1VerificationFacetV2
    let barz: Barz
    ...
    let secp256r1VerificationFacetV2: Secp256r1VerificationFacetV2
    ...
    const setupR1Barz = async (keyPair: any, publicKeyBytes: any) => {
        ...
        secp256r1VerificationFacetV2 = await secp256r1VerificationFacetV2Fixture()
        ...
        await facetRegistry.connect(facetRegistryOwner).registerFacetFunctionSelectors(secp256r1VerificationFacetV2.address, getSelectors(secp256r1VerificationFacetV2))
        ...
        barz = await barzFixture(accountFacet, secp256r1VerificationFacetV2, entryPoint, facetRegistry, defaultFallbackHandler, publicKeyBytes)

        await entryPoint.depositTo(barz.address, {
            value: ethers.utils.parseEther('0.5'),
        })

        r1Barz = await getFacetBarz("Secp256r1VerificationFacetV2", barz)
    ...
    describe("# uninitializeSigner", () => {
        it("Should revert uninitialization if not middle of signature migration", async () => {
            ...
            await expect(r1Barz.uninitializeSigner()).to.be.revertedWithCustomError(r1Barz, "LibAppStorage__AccountMustBeUninitialized")
        })
    })
    describe("# validateOwnerSignature", () => {
        it("Should verify valid secp256r1 (passkeys) signature, which is pre-signed using https://webauthn.me", async () => {
            ...
            const { barz } = await setupContracts(facetRegistryOwner, securityManagerOwner, entryPoint, publicKeyBytes, guardianSecurityPeriod, guardianSecurityWindow, recoveryPeriod, lockPeriod, approvalValidationPeriod, migrationPeriod, true, true)
            r1Barz = await getFacetBarz("Secp256r1VerificationFacetV2", barz)
            const hexString = "0x9430dd40f06e792938e7c0cbfcaf64101c552923e0cd7a897b4f9e83330bb695"
            const originS = new BN(hexString.slice(2), 'hex')
            const sValue = adjustS(originS, ec.n)
            const signature = ethers.utils.defaultAbiCoder.encode(
                ...
                [
                    ...
                    ethers.BigNumber.from(sValue.toString()),
                    ethers.utils.concat([
    ...
    describe("EIP-1271 # isValidSignature", async () => {
        it("Should return EIP-1271 magic value if signature is valid", async () => {
            ...
            const ethSignMsgHash = keccak256_buffer(Buffer.concat([
                Buffer.from('\x19Ethereum Signed Message:\n32', 'ascii'),
                Buffer.from(ethers.utils.arrayify(msgHash))
            ]))

            const finalHash = await getMessageHash(ethSignMsgHash, await getChainId(), r1Barz.address)

            const opHashBase64 = base64url.encode(concat([finalHash]))
        ...
        it("Should return dummy value when signature is invalid", async () => {
            ...
            const finalHash = await getMessageHash(msgHash, await getChainId(), r1Barz.address)
